...
# --- INITIAL SETUP ---
st.set_page_config(page_title="PFR Applicant Checker", layout="wide")
st.title("🕵️ PFR Applicant Checker")
...
fuzzy_threshold = st.sidebar.slider("Near-Identical Pattern %", 80, 100, 95)
...
if resp_file and screen_file:
    try:
        ...
        for q_text in logic_df[q_col].unique():
            ...
            auto_rejected = q_rows[q_rows[so_col].astype(str).str.contains("Disqualify", case=False, na=False)][a_col].tolist() if so_col else []
        ...
        # --- STEP 4: CONSISTENCY ---
        st.header("⚖️ Step 2: Consistency")
        ...
        # --- STEP 5: RUN AUDIT ---
        if st.button("🚀 Run Appliacation Checker"):
            # A. GRADING
            def grade_row(row):
            ...
            if 'Mobile' in df_resp.columns:
                ...
                for idx in phone_dupes.index:
                    ...
                    df_resp.at[idx, 'Suspicion_Reasons'] += "Phone Cluster; "
            ...
            t1, t2, t3, t4 = st.tabs(["👥 Near-Identical Twins", "📱 Phone Clusters", "⚖️ Consistency Mismatches", "📥 Full Report"])

            with t1:
                st.subheader("Near-Identical Answer Patterns")
                if fuzzy_groups:
                ...
                else: st.success("No fuzzy answer patterns detected.")

            with t2:
                st.subheader("Phone Number Clusters")
